...
2.  **State Isolation & Context Fusion:** The graph uses a shared `ctx.state` object to
...
-   **Router Agent (agents/router_agent.py):** The gateway agent. It inspects the farmer's
    query to classify user intent (e.g. GENERAL_QUESTION, CROP_PLANNING, WEATHER_INQUIRY,
    MARKET_INQUIRY). It returns a structured JSON classification.
...
Cross-Signal Intelligence (3-Signal Fusion)
-------------------------------------------
The core intelligence layer relies on fusing 3 distinct signals in `compile_advisory_input`:
1.  **Weather conditions:** Current data + 7-day forecast + 14-day historical rain.
2.  **Market trends:** Spot commodity prices + 30-day historical trajectories.
3.  **Farm context:** The physical farm grid layout, crop plan, and historical performance.
Fusing these allows the Advisory Agent to make cross-domain insights. For example, if the
weather forecast predicts 80mm of rain (Weather Signal) and market prices for the crop are
falling (Market Signal), the agent will recommend *delaying* costly supplemental irrigation
to optimize the farmer's bottom line.
...
-   **Security Input Node (`security_input_validation`):** Runs immediately at `START` to
    truncate input length (guarding against cost bloating/token-stuffing) and detect prompt
...
def route_after_router(ctx: Context, node_input: str) -> str:
    """Parse the router's JSON classification and forward to the advisory signal chain."""
    router_text = str(node_input)
    ctx.state["router_output"] = router_text

    # Attempt to parse router intent from JSON
    try:
...
def compile_advisory_input(ctx: Context, node_input: str) -> str:
    ...
    prompt = (
        ...
        f"1. WEATHER DATA (current + 7-day forecast + historical rain):\n{_fmt(weather_out)}\n\n"
        f"2. MARKET PRICES (spot + 30-day trend for all farm crops):\n{_fmt(market_out)}\n\n"
        f"3. FARM CONTEXT (profile, grid, crop plan, indicators):\n{_fmt(farm_context)}\n\n"
...
root_workflow = Workflow(
    ...
    edges=[
        ...
        # All intents flow through the same signal-gathering chain:
        # route → dispatch → weather → market → compile → advisory
        (route_after_router, dispatch_advisory_signals),
